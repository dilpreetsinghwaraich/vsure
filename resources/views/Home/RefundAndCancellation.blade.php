<div class="header">
    <div class="bg-color" style="background: url('<?php echo asset('/public'); ?>/images/home-back.jpg')">
      <div class="wrapper">
        <div class="banner-info about-page-banner wow fadeIn delay-05s animated" style="visibility: visible; animation-name: fadeIn;">
          <h1 class="bnr-title  text-center gallery">Refund And Cancellation</h1>
          <h2 class="text-center gallery"><a href="<?php echo url('/'); ?>"><span>Home</span></a> - <span>Refund And Cancellation</span></h2>
        </div>
      </div>
    </div>
  </div>
  <!-- Feature start-->
  <section id="about-page-serve-you" class="section-padding wow fadeIn delay-05s animated" style="visibility: visible; animation-name: fadeIn;">
    <div class="container">
      <div class="about-page-serve-heading">
        <h1>We Are Here To Serve You</h1>
        <p>Get the services here</p>
        <hr class="bottom-line-left">
      </div>
      <div class="about-page-serve-data">
        <div class="row">
          <div class="col-lg-12">
            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12"> <img src="<?php echo asset('/public'); ?>/images/about-us-image.jpg"> </div>
            <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
              <p>We believe our strength and knowledge that differentiate us 
                from others Our 24 x 7 Support to clients from our CA asisted 
                team make our client as our family member. </p>
              <ul>
                <li>Neque porro quisquam est, qui dolorem ipsum quia dolor</li>
                <li>Neque porro quisquam est, qui dolorem ipsum quia dolor</li>
                <li>Neque porro quisquam est, qui dolorem ipsum quia dolor</li>
                <li>Neque porro quisquam est, qui dolorem ipsum quia dolor</li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!--feature end--> 
  